...
const WINDOWS_LAUNCHER_NAME = "service-sync-launcher.vbs";
...
// schtasks can only register a task that runs in the interactive session, so pointing the task
// action straight at the .cmd wrapper (a console program) makes a console window flash on every
// run — every 5 minutes — stealing focus from full-screen apps and games. wscript.exe is a
// GUI-subsystem host, so running the wrapper through this tiny .vbs with window style 0 performs
// the sync with no window at all, while still propagating the wrapper's exit code to Task Scheduler.
function renderWindowsLauncher(wrapperPath: string): string {
  const command = vbsQuote(`"${wrapperPath}"`);
  return `' Auto-generated by tokenmaxxing. Runs the sync wrapper without a console window.\r
Dim shell, exitCode\r
Set shell = CreateObject("WScript.Shell")\r
exitCode = shell.Run(${command}, 0, True)\r
  ...
  return Effect.tryPromise({
    try: async () => {
      ...
      if (paths.backend === "windows-task-scheduler") {
        await writeFileAtomic(windowsLauncherPath(paths), renderWindowsLauncher(paths.wrapperPath));
      } else {
...
function windowsTaskCreateArgs(paths: ServicePaths): string[] {
  return [
    ...
    "/TR",
    `wscript.exe //nologo //b ${cmdQuote(windowsLauncherPath(paths))}`,
    "/F",
  ];
}
...
// VBScript string literals are double-quote delimited and escape an embedded quote by doubling it.
